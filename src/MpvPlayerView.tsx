...
export default React.forwardRef<MpvPlayerViewRef, MpvPlayerViewProps>(
  function MpvPlayerView(props, ref) {
    ...
    useImperativeHandle(ref, () => ({
      ...
      setAudioDelay: async (seconds: number) => nativeRef.current?.setAudioDelay(seconds),
      setVolumeBoost: async (percent: number) => nativeRef.current?.setVolumeBoost(percent),
      setDialogueBoost: async (enabled: boolean) => nativeRef.current?.setDialogueBoost(enabled),
      setMonoDownmix: async (enabled: boolean) => nativeRef.current?.setMonoDownmix(enabled),
